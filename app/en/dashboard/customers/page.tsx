...
export default function CustomersPage() {
  ...
  async function loadCustomers() {
    ...
    (data || []).forEach((order) => {
      const email =
        order.customer_email?.trim().toLowerCase() || "no-email";
  ...
  function getStatusLabel(status: string) {
    if (status === "Нова") return "New";
    if (status === "Очаква плащане") return "Awaiting payment";
    if (status === "Обработва се") return "Processing";
    if (status === "Изпратена") return "Shipped";
    if (status === "Доставена") return "Delivered";
    if (status === "Отказана") return "Cancelled";
  ...
  return (
    ...
    >
      <h1 className="mb-8 text-4xl font-bold">Customers</h1>
